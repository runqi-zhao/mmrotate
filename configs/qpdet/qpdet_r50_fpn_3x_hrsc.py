...
model = dict(
    ...
    rpn_head=dict(
        ...
        bbox_coder=dict(
            type='HBBSlidePointCoderXYRAB',
            target_means=[.0, .0, .0, .0, .0],
            target_stds=[1.0, 1.0, 1.0, 0.5, 0.5]),
        loss_cls=dict(
            type='CrossEntropyLoss', use_sigmoid=True, loss_weight=1.0),
        loss_bbox=dict(type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0)),
    roi_head=dict(
        type='OBBStandardRoIHeadXYRAB',
        ...
        bbox_head=dict(
            ...
            num_classes=1,
            bbox_coder=dict(
                type='OBB2OBBDeltaXYWHTCoderXYRAB',
                target_means=[0., 0., 0., 0., 0.],
